...
dirs = "sbin dev etc/init.d home proc sys tmp usr/bin usr/sbin usr/share/udhcpc var/log var/run config"
...
def config_kernel():
    AddCrossCompiler()
    check_call('make -C "%s" menuconfig CROSS_COMPILE=i486-unknown-linux-uclibc- ARCH=i386' % GetKernelDir(),
               shell=True)
